...
public class PlayerCombatFlightController : CombatFlightController, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        ...
        if (PlayerCombatController.Instance.PlayerShip != ShipController)
        {
        ...
        List<Vector3> flightPathOutline = GetFlightPathOutline(transform.position, transform.forward, 14);
        MovementEffectsController.Instance.ShowMoveableArea(flightPathOutline);
    }

    public void OnDrag(PointerEventData eventData)
    {
        if (PlayerCombatController.Instance.OurTurn == false || PlayerCombatController.Instance.PlayerShip != ShipController)
            return;
